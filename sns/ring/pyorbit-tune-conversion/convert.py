...
MADX file. I do not know why. This script adjusts the input tunes to MADX until 
the output tunes from PyORBIT are correct.
...
if __name__ == "__main__":
    ...
    output_dir_tmp = "outputs_tmp"
    os.makedirs(output_dir_tmp, exist_ok=True)

    output_dir = "outputs"
    os.makedirs(output_dir, exist_ok=True)

    file = open(os.path.join(output_dir_tmp, "notes.txt"), "w")
    file.write("MADX tunes to get correct PyORBIT tunes: \n")
    file.write("nux_madx = {}\n".format(result["nux_madx"]))
    file.write("nuy_madx = {}\n".format(result["nuy_madx"]))
    file.close()

    nux = result["nux_pyorbit"] + 6.0
    nuy = result["nuy_pyorbit"] + 6.0

    filename = os.path.join(output_dir, f"sns_ring_nux-{nux:0.2f}_nuy-{nuy:0.2f}.lat")
    subprocess.call(f"cp LATTICE.lat {filename}", shell=True)
    subprocess.call(f"mv LATTICE.lat {output_dir_tmp}", shell=True)
    ...
    for filename in garbage_filenames:
        subprocess.call( f"mv {filename} {output_dir_tmp}", shell=True)
